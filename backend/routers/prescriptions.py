...
from backend.models import User, UserRole, Patient, Prescription, Doctor
...
@router.post("/for-patient/{patient_id}", response_model=PrescriptionResponse)
def doctor_create_prescription(
    ...
    """Doctor writes a prescription for a specific patient after completing an appointment."""
    if user.role != UserRole.DOCTOR:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only doctors can write prescriptions for patients",
        )
